...
namespace StatusEffects.NetCode.Entities
{
    ...
    public partial struct StatusVariableUpdateSystem : ISystem
    {
        ...
        partial struct StatusVariableUpdateJob : IJobEntity
        {
            ...
            public void Execute([EntityIndexInQuery] int sortKey, Entity entity, in DynamicBuffer<StatusEffects> statusEffects, in StatusVariableUpdate statusVariableUpdate)
            {
                ...
                if (StatusFloatLookup.HasBuffer(entity))
                {
                    ...
                    for (int i = 0; i < statusFloatBuffer.Length; i++)
                        GetValue(ref statusFloatBuffer.ElementAt(i), statusEffects);
                }
                ...
                if (StatusIntLookup.HasBuffer(entity))
                {
                    ...
                    for (int i = 0; i < statusIntBuffer.Length; i++)
                        GetValue(ref statusIntBuffer.ElementAt(i), statusEffects);
                }
                ...
                if (StatusBoolLookup.HasBuffer(entity))
                {
                    ...
                    for (int i = 0; i < statusBoolBuffer.Length; i++)
                        GetValue(ref statusBoolBuffer.ElementAt(i), statusEffects);
                }
            ...
            public void GetValue(ref StatusFloats statusFloat, in DynamicBuffer<StatusEffects> statusEffects)
            {
                Effect effect;
                ...
                foreach (var seffect in statusEffects)
                {
                    for (int i = 0; i < seffect.Data.Value.Effects.Length; i++)
                    {
                        effect = seffect.Data.Value.Effects[i];

                        if (effect.Id != statusFloat.Id)
                            continue;

                        effectValue = seffect.Stacks * (effect.UseBaseValue ? seffect.Data.Value.BaseValue : effect.FloatValue);
            ...
            public void GetValue(ref StatusInts statusInt, in DynamicBuffer<StatusEffects> statusEffects)
            {
                Effect effect;
                ...
                foreach (var seffect in statusEffects)
                {
                    for (int i = 0; i < seffect.Data.Value.Effects.Length; i++)
                    {
                        ...
                        effectValue = seffect.Stacks * (effect.UseBaseValue ? (int)seffect.Data.Value.BaseValue : effect.IntValue);
            ...
            public void GetValue(ref StatusBools statusInt, in DynamicBuffer<StatusEffects> statusEffects)
            {
                Effect effect;
                ...
                foreach (var seffect in statusEffects)
                {
                    for (int i = 0; i < seffect.Data.Value.Effects.Length; i++)
                    {
                        ...
                        effectValue = effect.UseBaseValue ? Convert.ToBoolean(seffect.Data.Value.BaseValue) : effect.BoolValue;
